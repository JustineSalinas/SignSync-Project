...
# We will use Gemini 1.5 Flash - it is incredibly fast and perfect for real-time translation
...
async def translate_signs(request: TranslationRequest):
    ...
    try:
        # Send the raw words to Gemini
        response = model.generate_content(
            f"Raw signs: {raw_signs}",
            generation_config=genai.types.GenerationConfig(
                temperature=0.3, # Keep it low for precise translation

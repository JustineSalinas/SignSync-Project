  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          The people behind <GradientText text="SignSync" animate={true} />
        </motion.h1>
        ...
        <div style={{ maxWidth: 1100, width: '100%', display: 'flex', flexWrap: 'wrap', gap: '4rem', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '1 1 400px', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
              ...
              <span style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#7c3aed' }}>For Organizations</span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '3rem' }}>
              {[
                { icon: <MonitorSmartphone size={18} />, color: '#a78bfa', bg: 'rgba(139,92,246,0.1)', title: 'Plug-and-play setup', desc: 'Works on any browser-enabled display. No app installation, no IT overhead required.' },
                { icon: <Lock size={18} />, color: '#34d399', bg: 'rgba(16,185,129,0.1)', title: '100% on-device processing', desc: 'Patient and visitor data never leaves the room — fully HIPAA and GDPR aligned.' },
                { icon: <BarChart3 size={18} />, color: '#fbbf24', bg: 'rgba(245,158,11,0.1)', title: 'Usage analytics dashboard', desc: 'Track interaction volume and language coverage across all your locations.' }
              ].map((feature, i) => (
          ...
          <div style={{ flex: '1 1 480px', background: isDark ? '#080510' : '#1a1030', borderRadius: 24, padding: '3.5rem 3rem', border: `1px solid ${isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.08)'}`, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3.5rem', marginBottom: '4.5rem' }}>
              <div><div style={{ fontFamily: SERIF, fontSize: '2.5rem', color: '#d946ef', lineHeight: 1, marginBottom: '0.75rem' }}>24+</div><div style={{ fontSize: '0.75rem', color: '#7a7088' }}>Institutions using SignSync</div></div>
              <div><div style={{ fontFamily: SERIF, fontSize: '2.5rem', color: '#10b981', lineHeight: 1, marginBottom: '0.75rem' }}>97%</div><div style={{ fontSize: '0.75rem', color: '#7a7088' }}>Gesture accuracy rate</div></div>
              <div><div style={{ fontFamily: SERIF, fontSize: '2.5rem', color: '#f59e0b', lineHeight: 1, marginBottom: '0.75rem' }}><CountingNumber value={80} prefix="<" suffix="ms" /></div><div style={{ fontSize: '0.75rem', color: '#7a7088' }}>Real-time latency</div></div>
              <div><div style={{ fontFamily: SERIF, fontSize: '2.5rem', color: '#8b5cf6', lineHeight: 1, marginBottom: '0.75rem' }}>0kb</div><div style={{ fontSize: '0.75rem', color: '#7a7088' }}>Data sent to servers</div></div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {[
                { icon: <Building2 size={14} />, name: 'Hospitals & Clinics', badge: 'LIVE', badgeColor: '#10b981' },
                { icon: <Landmark size={14} />, name: 'Government Counters', badge: 'LIVE', badgeColor: '#10b981' },
                { icon: <BarChart3 size={14} />, name: 'Banks & Financial Services', badge: 'PILOT', badgeColor: '#f59e0b' },
                { icon: <GraduationCap size={14} />, name: 'Educational Institutions', badge: 'PILOT', badgeColor: '#f59e0b' }
              ].map((type) => (
      ...
      <footer style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 2rem 2.5rem', gap: '0.5rem', borderTop: `1px solid ${isDark ? darkTheme.border : lightTheme.border}` }}>
        <p style={{ fontSize: '0.78rem', color: isDark ? darkTheme.textSubtle : '#9d95b4', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          <Shield size={13} style={{ opacity: 0.6 }} /> Camera access is required. Your privacy is protected — no data leaves this device.
        </p>
